...
if(isset($_GET['id']) && isset($_GET['code'])) {
  ...
  $stmt = $db->runQuery("SELECT * FROM tutors WHERE tutor_ID=:id AND tokenCode=:code");
  ...
  if($stmt->rowCount() == 1) {
    if(isset($_POST['btn-reset-pass'])) {
      ...
      if($cpass!==$pass) {
        ...
        $stmt = $db->runQuery("UPDATE tutors SET password=:password WHERE tutor_ID=:id");
        $stmt->execute(array(":password"=>$cpass,":id"=>$rows['tutor_ID']));
...
<html>
  ...
  <body id="login">
   ...
   <strong>Hello Tutor <?php echo $rows['tutor_Fname'] ?>!</strong> <br /> 
